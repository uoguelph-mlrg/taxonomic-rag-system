...
--------
    Run the script from the command line with arguments:
        ```python
        rare_species_simp_caption.py --output < path1 > --write
        ```
"""
...
async def main() -> None:
    ...
    if write:
        ...
        final_metrics_csv_name = str(
            output_path + f"LA_simpCAP_qwen_tax_metrics_{current_date}_{current_time}.csv"
        )
        predictions_csv_name = str(
            output_path + f"LA_simpCAP_qwen_predictions_{current_date}_{current_time}.csv"
        )
